...
test.describe("Model Selector", () => {
  ...
  test("can search for models", async ({ page }) => {
    ...
    // Should show at least one Gemini model
  ...
  test("shows model provider groups", async ({ page }) => {
    ...
    // Should show Google provider group header (now the only provider)
  ...
  test("can select a different model", async ({ page }) => {
    ...
    await page.getByText("Gemini 2.5 Flash").first().click();
    ...
    await expect(
      page.locator("button").filter({ hasText: "Gemini 2.5 Flash" }).first()
    ).toBeVisible();
  });
});
